...
import { mainAdmin } from "../data/mainAdmin";

const ProfilePage = () => {
  const { adminName, updateAdminName } = useContext(UserContext);

  // Initialize state from mainAdmin.js
  const [name, setName] = useState(mainAdmin.name || adminName || "");
  const [email, setEmail] = useState(mainAdmin.email || "");
  const [phone, setPhone] = useState(mainAdmin.phone || "");
  const [role, setRole] = useState(mainAdmin.role || "");
  const [location, setLocation] = useState(mainAdmin.location || "");
  const [gender, setGender] = useState(mainAdmin.gender || "");
  const [dob, setDob] = useState(mainAdmin.dob || "");
  const [joinedDate, setJoinedDate] = useState(mainAdmin.joinedDate || "");
  const [bio, setBio] = useState(mainAdmin.bio || "");
  const [profilePic, setProfilePic] = useState(mainAdmin.profilePic || "");
