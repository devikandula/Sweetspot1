...
const Feedback = () => {
  ...
  // Default to admin's location, fallback "Hyderabad"
  ...
  // Show feedbacks only for the admin's branch
  const filteredFeedbacks = allFeedbacks.filter(
    (f) => f.location === selectedCity
  );
  ...
  return (
    <div className="px-6 py-4">
      <div className="flex justify-between items-center mb-6">
        ...
        {/* Branch Dropdown (locked to admin’s location) */}
        <select
          value={selectedCity}
        ...
        {filteredFeedbacks.length > 0 ? (
          filteredFeedbacks.map((fb, idx) => (
            ...
            >
              ...
              <p className="mt-2 text-[rgba(87,84,84,0.7)] italic">
                {fb.message}
              </p>
            </div>
          ))
